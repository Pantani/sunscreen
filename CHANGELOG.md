...
- Multi-channel distribution pipeline. Tag pushes (`vX.Y.Z`) now fan out from
  the GitHub Release to three additional channels in parallel:
  - **Homebrew tap** (`Pantani/homebrew-sunscreen`) — formula generated by
    cargo-dist and committed to the tap on each release.
  - **Snap Store** (`sunscreen-cli`, classic confinement) — built for amd64
    and arm64, published to the `stable` channel.
  - **APT via Cloudsmith** (`pantani/sunscreen`) — `cargo-deb` produces
    amd64/arm64 packages and uploads them with idempotent `republish`.
- Per-channel `continue-on-error` plus a `release-summary` job that aggregates
  status into the workflow Job Summary — a broken channel never blocks the
  release.
- `docs/reference/distribution.md` documenting install commands, required
  secrets, rotation policy, and dry-run procedure.
...
- `sunscreen` is distributed through GitHub Releases, Homebrew (tap), Snap
  Store, and APT (Cloudsmith). crates.io, `cargo-binstall`, and Windows
  installers remain future Phase 8 work.
