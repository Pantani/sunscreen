...
`sunscreen chain doctor` (shipped in Phase 2 R4 and hardened in R5, `src/cli/chain.rs::run_doctor`) validates markers across the entire workspace and offers `--fix-markers` for recoverable cases: appendable marker hosts are repaired at EOF, a missing `dispatch` block is reinserted inside `#[program]` when instruction files provide enough data, and missing `error_variants` markers are rewrapped inside the existing `#[error_code]` enum. Note: the top-level `sunscreen doctor` is the **toolchain** diagnostic (verifies `anchor`/`solana`/`cargo`/etc.); workspace-level marker auditing lives under `chain doctor` per ADR-0001 §6.1.
